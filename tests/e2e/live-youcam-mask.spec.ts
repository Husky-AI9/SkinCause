...
test("renders live YouCam concern masks for the prepared synthetic face", async ({ page }, testInfo) => {
  ...
  const redness = page.getByRole("button", { name: "Visible redness pattern" });
  await expect(redness).toBeVisible();
  await expect(page.getByRole("button", { name: "Visible blemish pattern" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Texture variation" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Pore visibility" })).toBeVisible();
  await expect(page.getByText("Facial segmentation", { exact: true })).toBeVisible();
  await expect(page.getByText("7 provider mask overlays", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText(/\[youcam\] response normalized: .*masks=7/)).toBeVisible();
  const scoreSummary = page.getByTestId("provider-score-summary");
  await expect(scoreSummary.locator(".scan-score-row")).toHaveCount(7);
  await expect(scoreSummary.getByText("Pore visibility", { exact: true })).toBeVisible();
  await expect(scoreSummary.getByText("Texture variation", { exact: true })).toBeVisible();
  await expect(scoreSummary.getByText("Visible redness pattern", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("img", { name: /Visible redness pattern visual pattern overlay/ })).toBeVisible();
  await page.getByRole("button", { name: "Texture variation" }).click();
  await expect(page.getByRole("img", { name: /Texture variation visual pattern overlay/ })).toBeVisible();
  await page.getByRole("button", { name: "Pore visibility" }).click();
  await expect(page.getByRole("img", { name: /Pore visibility visual pattern overlay/ })).toBeVisible();
